...
原数据集包含 1470 名员工的信息，共 35 个字段，用于分析员工流失（Attrition）因素。这是由 IBM 数据科学家创建的**虚构数据集**，不涉及任何真实员工信息，可放心用于学习和研究。
...
- ✅ **本土化表达优化**：采用更符合中文HR领域的术语（如“岗位”替代“职位角色”、“专业”替代“教育领域”）
- ✅ **字段名全中文**：35个字段全部翻译为中文
...
- ✅ **Excel 友好**：输出为格式化的 Excel 文件，包含：
  ...
  - **编码列**：为所有数值型分类变量（如教育程度、满意度等）保留原始数值编码列（如“教育程度编码”），便于 Excel 透视和建模
  - **列顺序优化**：按国内HR阅读习惯排列，员工编号为首列，符合企业系统导出习惯
...
根据此许可证，您可以自由使用、修改和分享数据内容（包括商业用途），但必须遵守许可证条款（例如，不得对数据本身主张额外的版权限制）。
